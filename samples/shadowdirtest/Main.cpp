...
// floor->GetMaterial()->SetAlphaForSpecular(0);
    ...
    for (int i = 0; i < MAX_OBJS / 2; i++) {
        float z = -i * 2.5f;
        CreateObject(Mesh::GetOrCreate<SphereMesh>("Sphere"), Color::Red,
                     Vector3(-1, 1, z));
        // CreateObject(Mesh::GetOrCreate<BoxMesh>("Box"), COLOR_BLUE,
        // Vector3(1, 1, z));
    }

    for (int i = MAX_OBJS / 2; i < MAX_OBJS; i++) {
        float z = -i * 2.5f;
        // auto sphere = CreateObject(Mesh::GetOrCreate<SphereMesh>("Sphere"),
        // COLOR_RED, Vector3(-1, 1, z));
        // sphere->GetMaterial()->SetSpecularColor(COLOR_WHITE);
        CreateObject(Mesh::GetOrCreate<BoxMesh>("Box"), Color::Blue,
                     Vector3(1, 1, z));
    }
#else
...
// CreateObject(Mesh::Create<BoxMesh>(), COLOR_BLUE, Vector3(0, 1, 0));
    ...
    auto debugSlot = Renderer::GetPtr()->SigDebugRenderer()->Connect(
    ...
    auto slotUpdate = engine->SigUpdate()->Connect([&](float deltaTime) {
        // LOGI("%s", ToString(camera->GetView() * Vector4(0, 0, -50,
        // 1)).c_str());
