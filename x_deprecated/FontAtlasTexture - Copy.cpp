...
namespace NSG
{
	...
	viewHeight_(0)
	{
		if(filename_.empty())
		{
			pResource_ = PResource(new ResourceMemory((const char*)(&FREESANS[0]), FREESANS_SIZE));
		}
		else
		{
	...
	void FontAtlasTexture::CreateTextureAtlas(bool generateMipmaps)
	{
		...
		if (filename_.empty())
		{
			TRACE_LOG("FontAtlasTexture::CreateTextureAtlas: Internal FreeSans.inl fontSize=" << fontSize_);
		}
		else
		{
		...
		for(int i = 32; i < FontAtlasTexture::MAXCHARS; i++) 
		{
			...
			charInfo_[i].ax = (float)(g->advance.x >> 6);
			charInfo_[i].ay = (float)(g->advance.y >> 6);
			charInfo_[i].bw = (float)g->bitmap.width;
			charInfo_[i].bh = (float)g->bitmap.rows;
			charInfo_[i].bl = (float)g->bitmap_left;
			charInfo_[i].bt = (float)g->bitmap_top;
