...
static void Test01() {
    ...
    auto Decode = [](Vector3 depth) -> float {
        const Vector3 dotValues =
            Vector3(1.0f, 1.0f / 255.0f, 1.0f / (255.0f * 255.0f));
    ...
    auto Encode3 = [](float depth) -> Vector4 {
        const Vector4 bit_shift =
            Vector4(256.0 * 256.0 * 256.0, 256.0 * 256.0, 256.0, 1.0);
        const Vector4 bit_mask =
            Vector4(0.0, 1.0 / 256.0, 1.0 / 256.0, 1.0 / 256.0);
    ...
    auto Decode3 = [](Vector4 rgba_depth) -> float {
        const Vector4 bit_shift =
            Vector4(1.0 / (256.0 * 256.0 * 256.0), 1.0 / (256.0 * 256.0),
                    1.0 / 256.0, 1.0);
    ...
    auto Encode4 = [](float depth) -> Vector4 {
        const Vector4 bit_shift =
            Vector4(32.0 * 64.0 * 32.0, 64.0 * 32.0, 1.0, 0.0);
    ...
    auto Decode4 = [](Vector4 rgba_depth) -> float {
        const Vector4 bit_shift =
            Vector4(1.0 / (32.0 * 64.0 * 32.0), 1.0 / (64.0 * 32.0), 1.0, 0.0);
        float depth = rgba_depth.Dot(bit_shift);
        return depth;
    };
