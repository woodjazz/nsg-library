...
static void Test01() {
    ...
    CHECK_ASSERT(Distance(shadowCam->GetGlobalPosition(),
                          Vector3(0, 5009.f, 0)) < 0.001f);
...
static void Test02() {
    ...
    SignalKey::PSlot slotKey =
        window->SigKey()->Connect([&](int key, int action, int modifier) {
            ...
            for (int i = 0; i < nSplits; i++) {
                ...
                LOGI("ShadowCam pos = %s",
                     ToString(shadowCam->GetGlobalPosition()).c_str());
