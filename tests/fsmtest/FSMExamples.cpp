...
struct BaseState : State {
    ...
    void Begin() { printf("Begin %s\n", name_); }

    void Stay() { printf("Stay %s\n", name_); }

    void End() { printf("End %s\n", name_); }
};
...
public:
    ...
    const State* GetState() const { return fsm_.GetState(); }
...
struct BaseMachine : Machine {
    ...
    BaseMachine(State& initialState, const char* name)
        : Machine(initialState, false), name_(name) {}
...
public:
    Entity()
        : b1_(b11_), b_(b1_), c_(c1_), fsm_(a_), t1_(false), t2_(false),
          t3_(false) {
