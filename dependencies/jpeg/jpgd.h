...
// actual_comps will be set to the either 1 (grayscale) or 3 (RGB).
// Notes: For more control over where and how the source data is read, see the
// decompress_jpeg_image_from_stream() function below, or call the jpeg_decoder
// class directly.
// Requesting a 8 or 32bpp image is currently a little faster than 24bpp because
...
// memory. Set m_eof_flag to true when no more data is available.
// The decoder is rather greedy: it will keep on calling this method until its
// internal input buffer is full, or until the EOF flag is set.
// It the input stream contains data after the JPEG stream's EOI (end of image)
// marker it will probably be pulled into the internal buffer.
// Call the get_total_bytes_read() method to determine the actual size of the
// JPEG stream after successful decoding.
...
public:
    ...
    // remaining)
    // Returns -1 on error, otherwise return the number of bytes actually
...
public:
    ...
    // valid or not. You may call the get_width(), get_height(), etc.
    // methods after the constructor is called. You may then either destruct the
    // object, or begin decoding the image by calling begin_decoding(), then
    // decode() on each scanline.
    ...
    // pixels (get_bytes_per_pixel() will return 1).
    // Otherwise, it will always point to a buffer containing 32-bit RGBA pixels
    // (A will always be 255, and get_bytes_per_pixel() will return 4).
    // Returns JPGD_SUCCESS if a scan line has been returned.
    // Returns JPGD_DONE if all scan lines have been returned.
    // Returns JPGD_FAILED if an error occurred. Call get_error_code() for a
    // more info.
