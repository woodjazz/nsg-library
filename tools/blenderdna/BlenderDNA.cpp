...
namespace Blender
{
    ...
    void CalcDependencyLevel(const hgString& type, DNA* dna, size_t& level)
    {
        ++level;
        short* sp = findStruct(type.c_str(), dna);
        if (sp)
        {
            int len = sp[1];
            sp += 2;
            for (short el = 0; el < len; el++, sp += 2)
            {
                ...
                if (dn[0] != '*' && sp[0] >= dna->getStruct(0)[0])
                    CalcDependencyLevel(dt, dna, level);
            }
        }
    }
    ...
    void dumpStructs(FILE* fp, DNA* dna)
    {
        ...
        for (int i = 0; i < dna->getNumStructs(); i++)
        {
            ...
            StructName sn = { dna, type, findStruct(type.c_str(), dna), level, max, max2 };
    ...
    // ----------------------------------------------------------------------------
    int GenerateDNA(const Path& filePath, const Path& outputDir)
    {
        ...
        if (dnastart != -1)
        {
            ...
            Path outputHeader(outputDir);
            outputHeader.SetFileName("Blender.h");

            FILE* bfp = fopen(outputHeader.GetFullAbsoluteFilePath().c_str(), "wb");
            if (bfp)
            {
            ...
            Path outputCpp(outputDir);
            outputCpp.SetFileName("BlenderDNA.cpp");
            ...
            if (bfp)
            {
                ...
                fprintf(bfp, "\n};\n");
                fprintf(bfp, "int DNAlen=sizeof(DNAstr);\n");
                fclose(bfp);
            }
            #endif
