...
#define SplineTOL                std::numeric_limits<float>::epsilon()
...
namespace BlenderConverter
{
    ...
    float BSpline::interpolate(const double& t,
                                         const double& p0,
                                         const double& p1,
                                         const double& p2,
    ...
    int BSpline::solveRoots(const double& x,
                                   const double p0,
                                   const double p1,
                                   const double p2,
                                   const double p3,
                                   double& s) const
    {
        ...
        if (m_interpMethod == BSpline::BEZ_LINEAR)
        {
            // do linear
            if (SplineFuzzy(c1))
                return 0;
    ...
    void BSpline::updateHandles(float* p0, float* p1, float* p2, float* p3) const
    {
        ...
        if (lh != 0.0)
        {
            if (lh > (p3[0] - p0[0]))
            {
                ...
                p1[0] = (p0[0] - f * std::abs((p0[0] - p1[0])));
                p1[1] = (p0[1] - f * std::abs((p0[1] - p1[1])));
                p2[0] = (p3[0] - f * std::abs((p3[0] - p2[0])));
                p2[1] = (p3[1] - f * std::abs((p3[1] - p2[1])));
            }
        }
    }

    float BSpline::interpolate(float delta, float time) const
    {
        const BezierVertex* vp = &m_verts[0];
