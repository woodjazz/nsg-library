...
namespace BlenderConverter
{
	...
	BScene::BScene(const NSG::Path& path, const NSG::Path& outputDir, bool embedResources)
		: path_(path),
		outputDir_(outputDir),
		embedResources_(embedResources),
		defaultMaterial_(NSG::Material::GetOrCreate("__DefaultBlenderMaterial__"))
	{
	...
	void BScene::LoadScenes(bParse::bMain* data)
	{
#ifndef USING_BLENDER_FILES
		...
		for (int i = 0; i < n; i++)
		{
			...
			auto scene = LoadScene(bscene);
			ConvertGroups(data, scene, bscene);
			ConvertGroupInstances(scene, bscene);
		}
#endif
	}
	...
	void BScene::ConvertGroups(bParse::bMain* data, PScene scene, const Blender::Scene* bscene)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			ConvertGroup(bgroup, scene, bscene);
		}
#endif
	}
	...
	void BScene::CreateAnimations(bParse::bMain* data)
	{
		...
		for (int i = 0; i < n; i++)
		{
			const bAction* action = (const bAction*)it->at(i);
			CreateAnimation(action, firstBScene, firstScene);
		}
#endif
	}

	NSG::PScene BScene::LoadScene(const Blender::Scene* bscene)
	{
		physics_.clear();
		auto scene = CreateScene(bscene);
		scenes_.push_back(scene);
		bscenes_.push_back(bscene);
		const Base* base = (const Base*)bscene->base.first;
		while (base)
		{
	...
	{
		auto scene = std::make_shared<NSG::Scene>(B_IDNAME(bscene));
		scene->SetOrientation(glm::angleAxis<float>(-PI / 2.f, Vertex3(1, 0, 0)));
		const World* world = bscene->world;
		if (world)
		{
	...
	PMaterial BScene::LoadMaterial(const Blender::Material* mt)
	{
		...
		auto material = NSG::Material::GetOrCreate(name);
	...
	void BScene::ConvertGroup(Blender::Group* bgroup, PScene scene, const Blender::Scene* bscene)
	{
		...
		auto bgobj = (GroupObject*)bgroup->gobject.first;
		while (bgobj)
		{
	...
	void BScene::ConvertGroupInstances(PScene scene, const Blender::Scene* bscene)
	{
		...
		for (auto& group : groups)
		{
			...
			if (!instancedGroup)
				continue;
			const Group* bgobj = group->dup_group; // Owning group
	...
	PSceneNode BScene::ConvertObject(const Blender::Object* obj, PScene scene, const Blender::Scene* bscene)
	{
		...
		if (obj && IsKnownType(obj))
		{
			...
			case OB_ARMATURE:   // SceneNode + Skeleton
				sceneNode = CreateSkeletonBones(obj, parent, bscene);
				break;
			case OB_CURVE:
				break;
			default:
	...
	void BScene::GetFrames(const bAction* action, std::vector<float>& fra)
	{
		const FCurve* fcu = (const FCurve*)action->curves.first;
	...
	float BScene::GetTracks(const bAction* action, float animfps, BTracks& tracks)
	{
		...
		const FCurve* bfc = (const FCurve*)action->curves.first;
	...
	void BScene::CreateAnimation(const Blender::bAction* action, const Blender::Scene* bscene, PScene scene)
	{
		float sec_base = 1.f;
		float frs_sec = 24.f;
		if (bscene->r.frs_sec_base != 0)
			sec_base = bscene->r.frs_sec_base;
		if (bscene->r.frs_sec != 0)
			frs_sec = bscene->r.frs_sec;
		float animfps = frs_sec / sec_base;
		std::string name(B_IDNAME(action));
		if (!Animation::Has(name))
		{
			auto anim = Animation::Create(name);
			BTracks tracks;
	...
	void BScene::ConvertTracks(PScene scene, const bAction* action, PAnimation anim, BTracks& tracks, float length)
	{
		for (auto& btrack : tracks)
		{
			std::string channelName = btrack.first;
			AnimationTrack track;
			track.nodeName_ = channelName;
#if 0
			if (scene->GetName() == channelName)
				track.node_ = scene;
			else
				track.node_ = GetBoneNode(channelName);
	...
	AnimationChannelMask BScene::ConvertChannel(const bAction* action, PTrackData trackData, AnimationTrack& track, float timeFrameLength)
	{
	...
	PBSpline BScene::ConvertSpline(const BezTriple* bez, SPLINE_CHANNEL_CODE access, int mode, int totvert, float xoffset, float xfactor, float yoffset, float yfactor)
	{
	...
	void BScene::GetActionStartEnd(const bAction* action, float& start, float& end)
	{
		start = std::numeric_limits<float>::max();
		end = -start;
		float tstart, tend;
		FCurve* bfc = (FCurve*)action->curves.first;
		while (bfc)
		{
	...
	void BScene::GetSplineStartEnd(const BezTriple* bez, int totvert, float& start, float& end)
	{
		start = std::numeric_limits<float>::max();
		end = -start;
		const BezTriple* bezt = bez;
	...
	{
		return obj->gameflag & OB_BOUNDS ? true : false;
	}
	...
	{
		PhysicsBody bodyType = GetBodyType(obj->body_type);
		//const RigidBodyOb* rigidbody_object = obj->rigidbody_object;
		//const RigidBodyCon* rigidbody_constraint = obj->rigidbody_constraint;
	...
	PSceneNode BScene::CreateCamera(const Blender::Object* obj, PSceneNode parent, const Blender::Scene* bscene)
	{
		...
		auto camera = parent->CreateChild<NSG::Camera>(B_IDNAME(obj));
	...
	PSceneNode BScene::CreateLight(const Blender::Object* obj, PSceneNode parent)
	{
		...
		Lamp* la = static_cast<Lamp*>(obj->data);
	...
	PSceneNode BScene::CreateMesh(const Blender::Object* obj, PSceneNode parent)
	{
		...
		do
		{
			...
			auto mesh = NSG::Mesh::GetClass<ModelMesh>(meshName);
			bool meshOk = true;
			if (!mesh)
			{
				mesh = NSG::Mesh::CreateClass<ModelMesh>(meshName);
	...
	const Blender::Object* BScene::GetAssignedArmature(const Blender::Object* obj) const
	{
		...
		{
			const ModifierData* mod = (const ModifierData*)obj->modifiers.first;
			for (; mod; mod = mod->next)
				if (mod->type == eModifierType_Armature)
					ob_arm = ((const ArmatureModifierData*)mod)->object;
		}
		return ob_arm;
	}
	...
	void BScene::GetBoneIndexByDeformGroupIndex(const Blender::Object* obj, const Blender::Object* obAr, std::vector<std::pair<int, std::string>>& list)
	{
		list.clear();
		int idx = 0;
		const bDeformGroup* def = (const bDeformGroup*)obj->defbase.first;
		while (def)
		{
	...
	const void* BScene::FindByString(const ListBase* listbase, const char* id, int offset) const
	{
		const Link* link = (const Link*)listbase->first;
		const char* id_iter;
		while (link)
		{
	...
	const bPoseChannel* BScene::GetPoseChannelByName(const bPose* pose, const char* name) const
	{
		if (!name || (name[0] == '\0'))
			return nullptr;

		return (const bPoseChannel*)FindByString(&(pose)->chanbase, name, offsetof(bPoseChannel, name));
	}

	const Blender::Bone* BScene::GetBoneFromDefGroup(const Blender::Object* obj, const bDeformGroup* def) const
	{
		const bPoseChannel* pchan = GetPoseChannelByName(obj->pose, def->name);
		return pchan ? pchan->bone : nullptr;
	}

	bool BScene::IsBoneDefGroup(const Blender::Object* obj, const bDeformGroup* def) const
	{
		return GetBoneFromDefGroup(obj, def) != nullptr;
	}

	void BScene::CreateOffsetMatrices(const Blender::Object* obAr, const Blender::Object* objMesh, PSceneNode armatureNode, PSkeleton skeleton)
	{
		const bPose* pose = obAr->pose;
		const bDeformGroup* def = (const bDeformGroup*)objMesh->defbase.first;
		while (def)
		{
			if (IsBoneDefGroup(obAr, def))
			{
				const bPoseChannel* pchan = GetPoseChannelByName(pose, def->name);
				PSceneNode bone = armatureNode->GetChild<SceneNode>(pchan->bone->name, true);
				Matrix4 offsetMatrix = glm::inverse(Matrix4(ToMatrix(pchan->bone->arm_mat)));
				skeleton->SetBoneOffsetMatrix(pchan->bone->name, offsetMatrix);
			}
			def = def->next;
		}
	}

	const Blender::Object* BScene::GetMesh(const Blender::Object* objArmature, const Blender::Scene* bscene) const
	{
		const Base* base = (const Base*)bscene->base.first;
		while (base)
		{
			...
			if (!(obj->restrictflag & OB_RESTRICT_RENDER)) // object-level restrictions in OUTLINER
			{
				if (obj->type == OB_MESH && obj->parent == objArmature)
				{
					CHECK_ASSERT(obj->parent->type == OB_ARMATURE, __FILE__, __LINE__);
					return obj;
				}
			}
			base = base->next;
	...
	PSceneNode BScene::CreateSkeletonBones(const Blender::Object* objArmature, PSceneNode parent, const Blender::Scene* bscene)
	{
		const bArmature* ar = static_cast<const bArmature*>(objArmature->data);
		...
		auto skeleton = Skeleton::GetOrCreate(armatureName);
		auto sceneNode = CreateSceneNode(objArmature, parent);
		sceneNode->SetSkeleton(skeleton);
		if (skeleton->IsEmpty())
		{
			std::vector<std::string> boneList;
			std::vector<std::pair<int, std::string>> jointList;
			const Blender::Object* objMesh = GetMesh(objArmature, bscene);
			GetBoneIndexByDeformGroupIndex(objMesh, objArmature, jointList);
			for (auto& joint : jointList)
				if (joint.first != -1)
					boneList.push_back(joint.second);
			skeleton->SetShaderOrder(boneList);
			CreateOffsetMatrices(objArmature, objMesh, sceneNode, skeleton);
			...
			{
				if (!bone->parent)
					BuildBoneTree(bone, nullptr, skeleton);
				bone = bone->next;
			}
		}
		return sceneNode;
	...
	void BScene::BuildBoneTree(const Blender::Bone* cur, PNode parent, PSkeleton skeleton)
	{
		...
		PBone bone;
		if (parent)
			bone = parent->CreateChild<NSG::Bone>(cur->name);
		else
			skeleton->AddRootBone(bone = std::make_shared<NSG::Bone>(cur->name));
		...
		while (chi)
		{
			BuildBoneTree(chi, bone, skeleton);
			chi = chi->next;
		}
	}
	...
	bool BScene::ConvertMeshLegacy(const Blender::Object* obj, const Blender::Mesh* me, PModelMesh mesh, int materialIndex)
	{
		...
		MTFace* mtface[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
		...
		{
			const MFace& curface = me->mface[fi];
	...
	bool BScene::ConvertMesh(const Blender::Object* obj, const Blender::Mesh* me, PModelMesh mesh, int materialIndex)
	{
		...
		// UV-Layer-Data
		MLoopUV* muvs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
		char* uvNames[8] = { nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr };
		...
		{
			const MPoly& curpoly = me->mpoly[fi];
	...
	int BScene::GetUVLayersBMmesh(const Blender::Mesh* mesh, MLoopUV** uvEightLayerArray, char** uvNames)
	{
		CHECK_ASSERT(mesh, __FILE__, __LINE__);

		int validLayers = 0;
		CustomDataLayer* layers = (CustomDataLayer*)mesh->ldata.layers;
		if (layers)
		{
			for (int i = 0; i < mesh->ldata.totlayer && validLayers < 8; i++)
			{
				...
				{
					MLoopUV* mtf = (MLoopUV*)layer.data;
					if (mtf)
					{
	...
	int BScene::GetUVLayersBMmeshLegacy(const Blender::Mesh* mesh, MTFace** eightLayerArray, char** uvNames, MCol** oneMCol)
	{
		CHECK_ASSERT(mesh, __FILE__, __LINE__);

		int validLayers = 0;

		CustomDataLayer* layers = (CustomDataLayer*)mesh->fdata.layers;
		if (layers)
		{
			for (int i = 0; i < mesh->fdata.totlayer && validLayers < 8; i++)
			{
				...
				{
					MTFace* mtf = (MTFace*)layers[i].data;
					if (mtf)
					{
						uvNames[validLayers] = layer.name;
				...
				{
					if (oneMCol && !(*oneMCol))
						*oneMCol = static_cast<MCol*>(layer.data);
				}
			}
		}
		else
	...
	void BScene::SetMaterial(const Blender::Object* obj, PSceneNode sceneNode, int materialIndex)
	{
		...
		std::string name = B_IDNAME(mt);
		auto material = NSG::Material::Get(name);
		if (material)
	...
	{
		NSG::Path outputFile(outputDir_);
	...
	void BScene::GenerateXML(pugi::xml_document& doc)
	{
		...
		NSG::Mesh::SaveMeshes(appNode);
		NSG::Material::SaveMaterials(appNode);
		Shape::SaveShapes(appNode);
		Skeleton::SaveSkeletons(appNode);
		Animation::SaveAnimations(appNode);
