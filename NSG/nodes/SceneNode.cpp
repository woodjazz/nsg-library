...
SceneNode::SceneNode(const std::string& name)
    : Node(name), octant_(nullptr), worldBBNeedsUpdate_(true),
      serializable_(true), signalMeshSet_(new SignalEmpty()),
...
bool SceneNode::CanBeVisible() const { return mesh_ != nullptr; }
...
PRigidBody SceneNode::GetOrCreateRigidBody() {
    ...
    if (!rigidBody_)
        rigidBody_ = std::make_shared<RigidBody>(
            std::dynamic_pointer_cast<SceneNode>(SharedFromPointerNode(this)));
    return rigidBody_;
}
...
PAnimationController SceneNode::GetOrCreateAnimationController() {
    if (!animationController_)
        animationController_ = std::make_shared<AnimationController>(
            std::dynamic_pointer_cast<SceneNode>(SharedFromPointerNode(this)));
...
void SceneNode::Save(pugi::xml_node& node) const {
    ...
    if (skeleton_)
        node.append_attribute("skeleton")
            .set_value(skeleton_->GetName().c_str());
    node.append_attribute("nodeType").set_value("SceneNode");
    if (material_)
        node.append_attribute("materialName")
            .set_value(material_->GetName().c_str());
    if (mesh_)
...
PSceneNode SceneNode::GetArmature() const { return armature_.lock(); }
...
void SceneNode::SetSkeleton(PSkeleton skeleton) {
    if (skeleton_ != skeleton) {
        skeleton_ = skeleton;
        if (skeleton_) {
            auto thisSceneNode = std::dynamic_pointer_cast<SceneNode>(
                SharedFromPointerNode(this));
...
void SceneNode::FillShaderDefines(std::string& defines) const {
    ...
    if (armature) {
        ...
        if (nBones) {
            defines += "MAX_BONES " +
                       ToString(Bone::GetMaxPlatformBones(nBones)) + "\n";
            defines += "SKINNED\n";
        }
    }
}
