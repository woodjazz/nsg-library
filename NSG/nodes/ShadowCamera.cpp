...
ShadowCamera::ShadowCamera(Light* light)
    : Camera(light->GetName() + "ShadowCamera"), light_(light),
      farSplit_(Scene::MAX_WORLD_SIZE), disabled_(false) {
    dirPositiveX_.SetLocalLookAtPosition(Vector3(1.0f, 0.0f, 0.0f),
                                         Vector3(0.0f, -1.0f, 0.0f));
    dirNegativeX_.SetLocalLookAtPosition(Vector3(-1.0f, 0.0f, 0.0f),
                                         Vector3(0.0f, -1.0f, 0.0f));
    dirPositiveY_.SetLocalLookAtPosition(Vector3(0.0f, 1.0f, 0.0f),
                                         Vector3(0.0f, 0.0f, 1.0f));
    dirNegativeY_.SetLocalLookAtPosition(Vector3(0.0f, -1.0f, 0.0f),
                                         Vector3(0.0f, 0.0f, -1.0f));
    dirPositiveZ_.SetLocalLookAtPosition(Vector3(0.0f, 0.0f, 1.0f),
                                         Vector3(0.0f, -1.0f, 0.0f));
    dirNegativeZ_.SetLocalLookAtPosition(Vector3(0.0f, 0.0f, -1.0f),
                                         Vector3(0.0f, -1.0f, 0.0f));
...
void ShadowCamera::SetupSpot(const Camera* camera) {
    ...
    auto farZ = Clamp(light_->GetShadowClipEnd(), light_->GetShadowClipStart(),
                      light_->GetDistance());
    SetFarClip(farZ);
...
void ShadowCamera::SetupDirectional(const Camera* camera, float nearSplit,
                                    float farSplit) {
    farSplit_ = farSplit;
    ...
    auto pos = camera->GetGlobalPosition() -
               Scene::MAX_WORLD_SIZE * light_->GetLookAtDirection();
    SetPosition(pos);
    SetOrientation(light_->GetGlobalOrientation());
    {
    ...
    }
    {
        ...
        auto receiversBox =
            Camera::GetViewBox(camSplitFrustum.get(), scene, true, false);
        ...
        auto castersBox =
            Camera::GetViewBox(camSplitFrustum.get(), scene, false, true);
        if (!castersBox.IsDefined()) {
            disabled_ = true;
            return; // no casters for this split => nothing to do
        }
        disabled_ = false;
        receiversBox.Transform(
            GetView()); // transform receivers view box to shadowCam's space
        castersBox.Transform(
            GetView()); // transform casters view box to shadowCam's space
        auto zFar = -receiversBox.min_.z; // from cam point of view: set zFar to
                                          // more distance z receiver
        auto zNear =
            -castersBox.max_
                 .z; // from cam point of view: set zNear to closest z caster
...
bool ShadowCamera::GetVisiblesShadowCasters(
