...
Scene::Scene(const std::string& name)
    : SceneNode(name), mainCamera_(nullptr), ambient_(0.3f, 0.3f, 0.3f, 1.f),
      horizon_(0.f, 0.f, 0.f, 1.f), octree_(std::make_shared<Octree>()),
      ...
      signalUpdate_(new Signal<float>()), enableFog_(false),
      fogMinIntensity_(0), fogStart_(5), fogDepth_(25), fogHeight_(0) {
...
void Scene::SetWindow(PWindow window) {
    if (window_.lock() != window) {
        if (window) {
            slotMouseMoved_ =
                window->SigFloatFloat()->Connect([this](float x, float y) {
            ...
            slotMouseDown_ = window->SigMouseDown()->Connect(
                [this](int button, float x, float y) {
            ...
            slotMouseUp_ = window->SigMouseUp()->Connect(
                [this](int button, float x, float y) {
            ...
            slotMouseWheel_ =
                window->SigMouseWheel()->Connect([this](float x, float y) {
...
bool Scene::GetFastRayNodesIntersection(const Ray& ray,
...
bool Scene::GetPreciseRayNodesIntersection(
    const Ray& ray, std::vector<RayNodeResult>& result) const {
...
bool Scene::GetClosestRayNodeIntersection(const Ray& ray,
...
void Scene::GetVisibleNodes(const Camera* camera,
...
void Scene::GetVisibleNodes(const Frustum* frustum,
                            std::vector<SceneNode*>& visibles) const {
    for (auto& obj : octreeNeedsUpdate_)
        octree_->InsertUpdate(obj);
    octreeNeedsUpdate_.clear();
...
void Scene::SavePhysics(pugi::xml_node& node) const {
    ...
    child.append_attribute("gravity").set_value(
        ToString(physicsWorld_->GetGravity()).c_str());
    child.append_attribute("fps").set_value(physicsWorld_->GetFps());
    child.append_attribute("maxSubSteps")
        .set_value(physicsWorld_->GetMaxSubSteps());
}
...
void Scene::AddLight(Light* light) {
    ...
    std::sort(lights_.begin(), lights_.end(),
              [&](const Light* a, const Light* b) -> bool {
...
const std::vector<Light*>& Scene::GetLights() const { return lights_; }
...
void Scene::RemoveParticleSystem(ParticleSystem* ps) {
...
SceneNode* Scene::GetClosestNode(const Camera* camera, float screenX,
                                 float screenY) const {
...
void Scene::SetMainCamera(PCamera camera) { mainCamera_ = camera.get(); }
...
float Scene::GetFogMinIntensity() const { return fogMinIntensity_; }

float Scene::GetFogStart() const { return fogStart_; }

float Scene::GetFogDepth() const { return fogDepth_; }

float Scene::GetFogHeight() const { return fogHeight_; }
