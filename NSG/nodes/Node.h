...
public:
    ...
    void SetGUIRotation(const Vector3& guiRotation) {
        guiRotation_ = guiRotation;
    };
    ...
    void SetGlobalLookAtPosition(const Vertex3& lookAtPosition,
                                 const Vertex3& up = Vertex3::Up);
    void SetLocalLookAtPosition(const Vertex3& lookAtPosition,
                                const Vertex3& up = Vertex3::Up);
    Quaternion GetLookAtOrientation(const Vertex3& lookAtPosition,
                                    const Vertex3& up);
    void SetGlobalPositionAndLookAt(const Vertex3& newPosition,
    ...
    template <typename T>
    std::shared_ptr<T> GetChild(const std::string& name, bool recursive) {
    ...
    template <typename T>
    std::shared_ptr<T> GetOrCreateChild(const std::string& name) {
    ...
    template <typename T>
    std::shared_ptr<T> CreateChild(const std::string& name = GetUniqueName()) {
        auto obj = std::make_shared<T>(name);
        AddChild(obj);
        obj->OnCreated();
        return obj;
    ...
    template <typename T>
    std::shared_ptr<T> CreateChild(const std::string& name,
