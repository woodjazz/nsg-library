...
Quaternion::Quaternion(const Matrix4& m) { *this = Matrix3(m).QuatCast(); }

Quaternion::Quaternion(const Matrix3& m) { *this = m.QuatCast(); }
...
Quaternion Quaternion::Slerp(const Quaternion& b, float t) const {
    ...
    if (cosTheta > 1 - std::numeric_limits<float>::epsilon()) {
        ...
        return Quaternion(Mix(w, zb.w, t), Mix(x, zb.x, t), Mix(y, zb.y, t),
                          Mix(z, zb.z, t));
    } else {
...
float Quaternion::Yaw() const { return asin(-2 * (x * z - w * y)); }
...
Quaternion Quaternion::Normalize() const {
    ...
    return Quaternion(w * oneOverLen, x * oneOverLen, y * oneOverLen,
                      z * oneOverLen);
}
...
Quaternion Rotation(const Vector3& orig, const Vector3& dest) {
    ...
    if (cosTheta < -1 + std::numeric_limits<float>::epsilon()) {
        ...
        if (rotationAxis.Length2() <
            std::numeric_limits<float>::epsilon()) // bad luck, they were
                                                   // parallel, try again!
    ...
    return Quaternion(s * 0.5f, rotationAxis.x * invs, rotationAxis.y * invs,
                      rotationAxis.z * invs);
}
}
