...
Matrix4::Matrix4(float x1, float y1, float z1, float w1, float x2, float y2,
...
Matrix4::Matrix4(const Vector4& v0, const Vector4& v1, const Vector4& v2,
                 const Vector4& v3) {
    value[0] = v0;
    value[1] = v1;
    value[2] = v2;
...
Matrix4::Matrix4(const Quaternion& q) { *this = Matrix4(Matrix3(q)); }
...
Matrix4::Matrix4(float left, float right, float bottom, float top, float zNear,
                 float zFar) {
    Matrix4 m(1);
    m[0][0] = 2 / (right - left);
    m[1][1] = 2 / (top - bottom);
...
Vector3 Matrix4::Translation() const { return value[3]; }
...
Vector3 Matrix4::Scale() const {
    return Vector3(sqrtf(value[0][0] * value[0][0] + value[0][1] * value[0][1] +
                         value[0][2] * value[0][2]),
                   sqrtf(value[1][0] * value[1][0] + value[1][1] * value[1][1] +
                         value[1][2] * value[1][2]),
                   sqrtf(value[2][0] * value[2][0] + value[2][1] * value[2][1] +
                         value[2][2] * value[2][2]));
}
...
Vector4 Matrix4::Row(int index) const {
    return Vector4(value[0][index], value[1][index], value[2][index],
                   value[3][index]);
}

const Vector4& Matrix4::Column(int index) const { return value[index]; }
...
void Matrix4::Decompose(Vector3& position, Quaternion& q,
                        Vector3& scale) const {
    const Matrix4& m = *this;
    Vertex3 scaling(value[0].Length(), value[1].Length(), value[2].Length());
    Matrix3 tmp1(
        Matrix4().Scale(Vector3(1 / scaling.x, 1 / scaling.y, 1 / scaling.z)) *
        m);
    q = tmp1.QuatCast();
