...
Texture2D::Texture2D(PResource resource, const TextureFlags& flags)
    : Texture(resource, flags), pixels_(nullptr) {}

Texture2D::Texture2D(const std::string& name)
    : Texture(name), pixels_(nullptr) {}

Texture2D::~Texture2D() {}

GLenum Texture2D::GetTarget() const { return GL_TEXTURE_2D; }
...
PTexture Texture2D::CreateFrom(const pugi::xml_node& node) {
    ...
    TextureWrapMode wrapMode =
        ToTextureWrapMode(node.attribute("wrapMode").as_string());
    TextureFilterMode filterMode =
        ToTextureFilterMode(node.attribute("filterMode").as_string());
    TextureBlend blendType =
        ToTextureBlend(node.attribute("blendType").as_string());
