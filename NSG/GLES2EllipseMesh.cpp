...
namespace NSG
{
	GLES2EllipseMesh::GLES2EllipseMesh(float width, float height, int res, PGLES2Material pMaterial, GLenum usage, bool flatX, bool flatY, float radiusX, float radiusY) 
	...
	filled_(true)
	{
		...
		for (int i = 0; i < res; i++)
		{
			...
			vertexData.position_.x = cos(angle)*radiusX;
			vertexData.position_.y = sin(angle)*radiusY;
            ...
            if(flatY && width >= 1)
            {
	            ...
	            {
				    vertexData.position_.x += halfX - radiusX;
	            }
	            else if(angle > PI/2)
	            {
	                vertexData.position_.x -= halfX - radiusX;
            ...
            if(flatX && height >= 1)
            {
	            if(angle > 0 && angle < PI)
	            {
				    vertexData.position_.y += halfY - radiusY;
	            }
	            else if(angle > PI)
	            {
	                vertexData.position_.y -= halfY - radiusY;
