...
Character::Character(PSceneNode sceneNode)
    : Object(sceneNode->GetName() + "Character"), sceneNode_(sceneNode),
      world_(sceneNode->GetScene()->GetPhysicsWorld()),
      ghost_(new btPairCachingGhostObject), compoundShape_(new btCompoundShape),
      restitution_(0), friction_(DEFAULT_FRICTION), stepHeight_(0.6f),
      verticalMove_(0), jumpSpeed_(0), forwardSpeed_(0), angularSpeed_(0),
      gravity_(9.8f), collisionGroup_((int)CollisionMask::ALL),
      collisionMask_((int)CollisionMask::ALL), handleCollision_(false),
      shapeHalfWidth_(0), shapeHeight_(0), shapeHalfHeight_(0),
      shapeSphereRadius_(0), forwardAxis_(-Vector3::Forward),
      upAxis_(Vector3::Up), flying_(false) {
...
float Character::GetGroundHeightFrom(const Vector3& pos) const {
    ...
    PhysicsRaycastResult result = world_.lock()->SphereCastBut(
        this, pos, direction.Normalize(), shapeHalfWidth_, distance);
...
void Character::ApplyGravity(float deltaTime) {
    ...
    if (result.HasCollided()) {
        auto onGroundPos =
            result.position_.y + shapeHalfHeight_ + nodeColliderOffset_.y;
...
void Character::updateAction(btCollisionWorld* collisionWorld,
                             btScalar deltaTime) {
...
ICollision* Character::StepForwardCollides() const {
    ...
    if (flying_)
        stepUp = 0;
    auto result = Obstruction(stepForwardSourcePos_, stepForwardTargetPos_,
                              std::min(stepUp, shapeSphereRadius_));
...
void Character::StepForward(float deltaTime) {
    ...
    if (result.HasCollided()) {
        ...
        if (slidingDirection_.Length() > EPSILON) {
            auto result = Obstruction(stepForwardSourcePos_,
                                      stepForwardSourcePos_ + slidingDirection_,
                                      0.25f * shapeSphereRadius_);
            if (!result.HasCollided())
        ...
        if (std::abs(groundHeight - groundHeight2) < stepUp)
            stepForwardFinalPos_ =
                position +
                stepForward_; // ground is at the same height after stepping
        else if (groundHeight + stepUp > groundHeight2)
            stepForwardFinalPos_ = position + stepForward_; // stepping down
        else
            stepForwardFinalPos_ =
                position + stepForward_ + verticalStep; // stepping up
    } else {
...
bool Character::IsOnGround() const { return verticalMove_ == 0 && !flying_; }

void Character::debugDraw(btIDebugDraw* debugDrawer) {
    auto debugRenderer = Renderer::GetPtr()->GetDebugRenderer();
    debugRenderer->AddLine(stepForwardSourcePos_, stepForwardTargetPos_,
                           Color::Red);
    auto worldTrans = ghost_->getWorldTransform();
    auto position = ToVector3(worldTrans.getOrigin());
    debugRenderer->AddLine(
        position, position + 10.f * slidingDirection_.Normalize(), Color::Blue);
}

PhysicsRaycastResult Character::Obstruction(const Vector3& origin,
                                            const Vector3& targetPos,
                                            float radius) const {
    auto direction = targetPos - origin;
    auto distance = direction.Length();
    PhysicsRaycastResult result = world_.lock()->SphereCastBut(
        this, origin, direction.Normalize(), radius, distance);
    return result;
}
...
void Character::AddShape(PShape shape, const Vector3& position,
...
void Character::SetJumpSpeed(float speed) { jumpSpeed_ = speed; }
...
void Character::SetForwardSpeed(float speed) { forwardSpeed_ = speed; }

void Character::SetAngularSpeed(float speed) { angularSpeed_ = speed; }
...
void Character::AllocateResources() {
    ...
    owner->addCollisionObject(ghost_.get(), (int)collisionGroup_,
                              (int)collisionMask_);
...
SceneNode* Character::GetSceneNode() const { return sceneNode_.lock().get(); }
}
