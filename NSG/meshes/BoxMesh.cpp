...
BoxMesh::BoxMesh(const std::string& name)
    : ProceduralMesh(name), width_(0), height_(0), depth_(0), resX_(0),
      resY_(0), resZ_(0) {
    Set();
...
void BoxMesh::Set(float width, float height, float depth, int resX, int resY,
                  int resZ) {
...
GLenum BoxMesh::GetWireFrameDrawMode() const { return GL_LINES; }

GLenum BoxMesh::GetSolidDrawMode() const { return GL_TRIANGLES; }

size_t BoxMesh::GetNumberOfTriangles() const { return indexes_.size() / 3; }
