...
namespace NSG {
template <>
std::map<std::string, PWeakMesh> WeakFactory<std::string, Mesh>::objsMap_ =
    std::map<std::string, PWeakMesh>{};

Mesh::Mesh(const std::string& name, bool dynamic)
    : Object(name), boundingSphereRadius_(0), isStatic_(!dynamic),
      areTangentsCalculated_(false), serializable_(true),
...
void Mesh::AllocateResources() {
    ...
    CHECK_ASSERT(GetSolidDrawMode() != GL_TRIANGLES ||
                 indexes_.size() % 3 == 0);

    if (isStatic_)
        pVBuffer_ =
            PVertexBuffer(new VertexBuffer(vertexsData_, GL_STATIC_DRAW));
    else if (!pVBuffer_)
        pVBuffer_ =
            PVertexBuffer(new VertexBuffer(vertexsData_, GL_DYNAMIC_DRAW));
    else
    ...
    if (!indexes_.empty()) {
        if (isStatic_)
            ...
            pIBuffer_ =
                PIndexBuffer(new IndexBuffer(indexes_, GL_DYNAMIC_DRAW));
        else
    ...
    if (!indexesWireframe_.empty()) {
        if (isStatic_)
            pIWireBuffer_ = PIndexBuffer(
                new IndexBuffer(indexesWireframe_, GL_STATIC_DRAW));
        else if (!pIWireBuffer_)
            pIWireBuffer_ = PIndexBuffer(
                new IndexBuffer(indexesWireframe_, GL_DYNAMIC_DRAW));
        else
    ...
    for (auto& vertex : vertexsData_) {
        ...
        boundingSphereRadius_ =
            std::max(boundingSphereRadius_, vertex.position_.Length());
    }
...
void Mesh::Save(pugi::xml_node& node) {
    ...
    child.append_attribute("wireFrameDrawMode")
        .set_value(GetWireFrameDrawMode());
    ...
    for (auto& obj : vertexsData_) {
        ...
        vertexData.append_attribute("b").set_value(
            ToString(obj.bonesID_).c_str());
        vertexData.append_attribute("c").set_value(
            ToString(obj.color_).c_str());
        vertexData.append_attribute("n").set_value(
            ToString(obj.normal_).c_str());
        vertexData.append_attribute("p").set_value(
            ToString(obj.position_).c_str());
        vertexData.append_attribute("u").set_value(
            ToString(obj.uv_[0]).c_str());
        vertexData.append_attribute("v").set_value(
            ToString(obj.uv_[1]).c_str());
        vertexData.append_attribute("w").set_value(
            ToString(obj.bonesWeight_).c_str());
    }
    if (indexes_.size()) {
...
void Mesh::AddSceneNode(SceneNode* node) { sceneNodes_.insert(node); }

void Mesh::RemoveSceneNode(SceneNode* node) { sceneNodes_.erase(node); }

void Mesh::AddQuad(const VertexData& v0, const VertexData& v1,
                   const VertexData& v2, const VertexData& v3,
                   bool calcFaceNormal) {
    if (vertexsData_.size() + 4 <= std::numeric_limits<IndexType>::max()) {
        ...
        LOGW("Too many vertices. Limit is %d",
             std::numeric_limits<IndexType>::max());
    }
}

void Mesh::AddTriangle(const VertexData& v0, const VertexData& v1,
                       const VertexData& v2, bool averageFaceNormal) {
...
void Mesh::AddTriangle(const VertexData& v0, const VertexData& v1,
                       const VertexData& v2, const Vector3& normal) {
    auto idx0 = vertexsData_.size();
    if (idx0 + 2 <= std::numeric_limits<IndexType>::max()) {
        IndexType vidx = (IndexType)idx0;
        vertexsData_.push_back(v0);
...
const VertexData& Mesh::GetTriangleVertex(size_t triangleIdx,
