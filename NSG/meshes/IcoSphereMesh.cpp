...
namespace NSG {
IcoSphereMesh::IcoSphereMesh(const std::string& name) : ProceduralMesh(name) {
    Set();
...
GLenum IcoSphereMesh::GetWireFrameDrawMode() const { return GL_LINE_STRIP; }

GLenum IcoSphereMesh::GetSolidDrawMode() const { return GL_TRIANGLES; }
...
void IcoSphereMesh::AllocateResources() {
    ...
    indexes_ = Indexes{0,  1, 2, 0,  3, 1, 0,  4, 5, 1,  7, 6,  1,  6,  2,
                       1,  3, 7, 0,  2, 4, 0,  5, 3, 2,  6, 8,  2,  8,  4,
                       3,  5, 9, 3,  9, 7, 11, 6, 7, 10, 5, 4,  10, 4,  8,
                       10, 9, 5, 11, 8, 6, 11, 7, 9, 10, 8, 11, 10, 11, 9};
    ...
    for (auto iteration = 0; iteration < iterations_; iteration++) {
        ...
        for (size_t i = 0; i < size / 12; i++) {
            ...
            // make 1 vertice at the center of each edge and project it onto the
            // sphere
            VertexData vertexData;
    ...
    for (size_t i = 0; i < indexToSplit.size(); i++) {
        ...
        for (size_t j = 0; j < faces.size(); j++) {
            if (faces[j] == index) {
                ...
                if ((data[index1].uv_[0].x > 0.5f) ||
                    (data[index2].uv_[0].x > 0.5f))
                    faces[j] = newIndex;
            }
        }
    }
