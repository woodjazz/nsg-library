...
namespace NSG
{
    ...
    {
        if (radius_ != radius || height_ != height ||  rings_ != rings || capped_ != capped)
        {
            radius_ = radius;
            height_ = height;
            rings_ = rings;
    ...
    GLenum CylinderMesh::GetWireFrameDrawMode() const
    {
        return GL_LINE_STRIP;
    }

    GLenum CylinderMesh::GetSolidDrawMode() const
    {
        return GL_TRIANGLES;
    }
    ...
    void CylinderMesh::AllocateResources()
    {
        ...
        for (float ring = 0; ring <= rings_; ring++)
        {
            ...
            for (float seg = 0; seg <= segments; seg++)
            {
                ...
                vertexData.position_ = q_ * Vertex3(radius_ * x, y * height_, radius_ * z);
        ...
        if (capped_)
        {
            ...
            for (float seg = 0; seg <= segments; seg++)
            {
                ...
                vertexData.position_ = q_ * Vertex3(radius_ * x, height_, radius_ * z);
                vertexData.uv_[0] = Vertex2(0.5f * (x + 1), 1 - 0.5f * (z  + 1));
                data.push_back(vertexData);
            }
