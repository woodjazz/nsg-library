...
namespace NSG {
Renderer::Renderer()
    : context_(RenderingContext::Create()), scene_(nullptr), camera_(nullptr),
    ...
    FrameBuffer::Flags frameBufferFlags(
        (unsigned int)(FrameBuffer::COLOR | FrameBuffer::COLOR_USE_TEXTURE |
                       FrameBuffer::DEPTH |
                       FrameBuffer::Flag::DEPTH_USE_TEXTURE));
    filterFrameBuffer_ = PFrameBuffer(new FrameBuffer(
        GetUniqueName("RendererFilterFrameBuffer"), frameBufferFlags));
    frameBuffer_ = PFrameBuffer(new FrameBuffer(
        GetUniqueName("RendererFrameBuffer"), frameBufferFlags));
}

Renderer::~Renderer() {}

inline std::vector<SceneNode*>
Renderer::ExtractTransparent(const std::vector<SceneNode*>& objs) {
...
inline void Renderer::RemoveFrom(std::vector<SceneNode*>& from,
                                 const std::vector<SceneNode*>& objs) {
...
inline std::vector<SceneNode*>
Renderer::ExtractFiltered(const std::vector<SceneNode*>& objs) {
    std::vector<SceneNode*> result;
    for (auto& node : objs) {
        if (node->HasFilter())
            result.push_back(node);
...
void Renderer::GetLightedBatches(const std::vector<Batch>& iBatches,
                                 std::vector<const Batch*>& result) const {
...
void Renderer::SortTransparentBackToFront(std::vector<SceneNode*>& objs) {
    ...
    std::sort(objs.begin(), objs.end(),
              [&](const SceneNode* a, const SceneNode* b) -> bool {
...
void Renderer::GenerateBatches(std::vector<SceneNode*>& visibles,
    ...
    for (auto& node : visibles) {
        ...
        if (usedMaterial != material) {
            ...
            if (!materials.empty()) {
                ...
                std::sort(lastMaterialData.data_.begin(),
                          lastMaterialData.data_.end(),
    ...
    for (auto& material : materials) {
        ...
        for (auto& obj : material.data_) {
            bool limitReached =
                batches.size() &&
                batches.back().GetNodes().size() >= Batch::MaxNodesInBatch;
...
void Renderer::Draw(const Batch* batch, const Pass* pass, const Light* light,
                    const Camera* camera) {
    context_->SetMesh(batch->GetMesh());
    if (batch->AllowInstancing()) {
        if (context_->SetupProgram(pass, scene_, camera, nullptr,
                                   batch->GetMaterial(), light))
            context_->DrawInstancedActiveMesh(*batch, instanceBuffer_.get());
    } else {
        auto& nodes = batch->GetNodes();
        for (auto& node : nodes) {
            if (context_->SetupProgram(pass, scene_, camera, node,
                                       batch->GetMaterial(), light))
...
int Renderer::GetShadowFrameBufferSize(int split) const {
    ...
    static const int SplitMapSize[ShadowCamera::MAX_SPLITS] = {1024, 512, 256,
                                                               128};
    return SplitMapSize[split];
}

int Renderer::CalculateSplits(const Camera* camera,
                              float splits[ShadowCamera::MAX_SPLITS],
                              const BoundingBox& camFrustumViewBox,
                              const BoundingBox& receiversViewBox) const {
...
void Renderer::GenerateCubeShadowMap(const Camera* camera, const Light* light) {
    ...
    if (shadowFrameBuffer->IsReady()) {
        auto shadowCamera = light->GetShadowCamera(0);
        for (unsigned i = 0; i < (unsigned)CubeMapFace::MAX_CUBEMAP_FACES;
             i++) {
            TextureTarget face =
                (TextureTarget)(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i);
            shadowCamera->SetCurrentCubeShadowMapFace(face);
            auto camFrustum = camera->GetFrustum();
            auto intersection =
                camFrustum->IsPointInside(light->GetGlobalPosition());
            bool genShadowMap = Intersection::OUTSIDE != intersection ||
                                Intersection::OUTSIDE !=
                                    camFrustum->IsInside(BoundingBox(
                                        *shadowCamera->GetFrustum()));
            if (genShadowMap) {
                auto oldFrameBuffer =
                    context_->SetFrameBuffer(shadowFrameBuffer, face);
...
void Renderer::GenerateShadowMaps(const Camera* camera, const Light* light) {
    ...
    case LightType::SPOT: {
        ...
        auto intersection =
            camera->GetFrustum()->IsPointInside(light->GetGlobalPosition());
        if (Intersection::OUTSIDE != intersection)
            Generate2DShadowMap(0, light);
        else if (Intersection::OUTSIDE !=
                 camera->GetFrustum()->IsInside(
                     BoundingBox(*shadowCamera->GetFrustum())))
            Generate2DShadowMap(0, light);
        light->SetShadowSplits(1);
    } break;
    ...
    default: // DIRECTIONAL
    {
        ...
        auto receiversViewBox = Camera::GetViewBox(
            camFrustum.get(), light->GetScene().get(), true, false);
        float splits[ShadowCamera::MAX_SPLITS];
        int shadowSplits = CalculateSplits(
            camera, splits, camFullFrustumViewBox, receiversViewBox);
...
void Renderer::DebugPhysicsPass() {
    ...
    if (world) {
        ...
        if (!meshLines->IsEmpty()) {
            ...
            if (context_->SetupProgram(&debugPass_, scene_, camera_, nullptr,
                                       debugMaterial_.get(), nullptr))
                context_->DrawActiveMesh();
            debugRenderer->Clear();
        }
    }
    ...
    if (filterFrameBuffer_->IsReady()) {
        ...
        do {
            ...
            auto it = std::partition(
                filtered.begin(), filtered.end(), [&](SceneNode* obj) {
            ...
            context_->SetFrameBuffer(oldFrameBuffer);
            filter->SetTexture(MaterialTexture::DIFFUSE_MAP,
                               filterFrameBuffer_->GetColorTexture());
...
void Renderer::Render(const Pass* pass, Mesh* mesh, Material* material) {
    ...
    if (context_->SetupProgram(pass, nullptr, nullptr, nullptr, material,
                               nullptr))
        context_->DrawActiveMesh();
}

void Renderer::Render(const Pass* pass, const Scene* scene,
                      const Camera* camera, SceneNode* node,
                      const Light* light) {
    context_->SetMesh(node->GetMesh().get());
    if (context_->SetupProgram(pass, scene, camera, node,
                               node->GetMaterial().get(), light))
        context_->DrawActiveMesh();
}
...
void Renderer::Render(Window* window, Scene* scene, Camera* camera) {
    ...
    if (useFrameBuffer) {
        context_->SetFrameBuffer(
            nullptr); // use system framebuffer to show the texture
...
SignalDebugRenderer::PSignal Renderer::SigDebugRenderer() {
    static SignalDebugRenderer::PSignal signalDebugRenderer(
        new SignalDebugRenderer);
