...
Socket::Socket(int port, const char* serverName) : port_(port) {
...
void Socket::Bind() {
    int activate = 1;
    if (-1 == setsockopt(sockfd_, SOL_SOCKET, SO_REUSEADDR, (char*)&activate,
                         static_cast<int>(sizeof(activate))))
        throw std::runtime_error(GetStringError());
    if (-1 == ::bind(sockfd_, (sockaddr*)&addr_, sizeof(sockaddr)))
        throw std::runtime_error(GetStringError());
}

bool Socket::Select(int msec) { return Socket::Select(msec, sockfd_); }
...
bool Socket::SelectW(int msec) { return Socket::SelectW(msec, sockfd_); }
...
bool Socket::Connect() {
    ...
    int optVal = 1;
    if (-1 == setsockopt(sockfd_, IPPROTO_TCP, TCP_NODELAY, (char*)&optVal,
                         sizeof(optVal)))
        throw std::runtime_error(GetStringError());
    return true;
}
...
void Socket::Send(const std::string& buffer) { Socket::Send(sockfd_, buffer); }
...
void Socket::SetRecvTimeout(int fd, int timeoutSecs) {
    ...
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (char*)&optRecVal,
                         sizeof(optRecVal)))
        throw std::runtime_error(GetStringError());
#else
    struct timeval trcv;
...
#if defined(IS_TARGET_WINDOWS)
    int optRecVal = timeoutSecs * 1000; // ms
    if (-1 == setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (char*)&optRecVal,
                         sizeof(optRecVal)))
        throw std::runtime_error(GetStringError());
#else
    struct timeval tsnd;
...
#endif
    if (-1 ==
        (fd_ = (int)accept(obj.sockfd_, (sockaddr*)&theirAddr, &sockAddrSize)))
        throw std::runtime_error(GetStringError());
}
...
const char* Socket::GetHostName() {
    ...
    gethostname(hostName, HOST_NAME1_MAX);
    return hostName;
}
...
bool Socket::ValidPort(int port) { return !(port < 0 || port > 0xFFFF); }
