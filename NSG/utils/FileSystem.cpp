...
EMSCRIPTEN_BINDINGS(FSReady) { emscripten::function("FSReady", &FSReady); }
...
EMSCRIPTEN_BINDINGS(FSSaved) { emscripten::function("FSSaved", &FSSaved); }
#endif
...
std::string FileSystem::GetPreferencesPath(const std::string& org,
    ...
    if (!isFSReady) {
        if (!isFSInitializing) {
            ...
            emscripten_run_script(
                std::string("FS.mkdir('" + org + "')").c_str());
            emscripten_run_script(
                std::string("FS.mkdir('" + org + "/" + app + "')").c_str());
            emscripten_run_script(
                std::string("FS.mount(IDBFS, {}, '/" + org + "/" + app + "')")
                    .c_str());
    ...
    if (!path.empty()) {
#ifdef IS_TARGET_WINDOWS
        {
            if (::SetCurrentDirectoryA(path.c_str()) == FALSE) {
                LOGE("Failed to change directory to %s with error = %d",
...
bool FileSystem::SaveDocument(const Path& path, const pugi::xml_document& doc,
                              bool compress) {
    if (compress) {
        Path filename(path);
