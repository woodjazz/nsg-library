...
#define MAKEFOURCC(ch0, ch1, ch2, ch3)                                         \
    ((unsigned)(ch0) | ((unsigned)(ch1) << 8) | ((unsigned)(ch2) << 16) |      \
     ((unsigned)(ch3) << 24))
#endif
...
Image::Image(PResource resource)
    : Object(resource->GetName()), resource_(resource) {
    ResetState();
}
...
void Image::AllocateResources() {
    ...
    if (!compressed_ &&
        !RenderingContext::GetPtr()->IsTextureSizeCorrect(width_, height_))
        Resize2PowerOf2();
...
void Image::ReadGeneric() {
    ...
    imgData_ =
        stbi_load_from_memory(data, dataSize, &width_, &height_, &channels_, 0);
    if (!imgData_) {
        imgData_ = jpgd::decompress_jpeg_image_from_memory(
            data, dataSize, &width_, &height_, &channels_, 4);
        if (!imgData_) {
            LOGE("Resource=%s failed with reason:%s",
                 resource_->GetName().c_str(), stbi_failure_reason());
        }
    }

    if (imgData_) {
        CHECK_ASSERT((channels_ == 4 || channels_ == 3 || channels_ == 1) &&
...
void Image::ReadDDS() {
    ...
    CHECK_CONDITION(ddsd.ddpfPixelFormat_.dwFourCC_ != 0 &&
...
void Image::ReadKTX() {
    ...
    CHECK_CONDITION(endianness == 0x04030201 &&
                    "Big-endian KTX files not supported");
    CHECK_CONDITION(type == 0 && format == 0 &&
                    "Uncompressed KTX files not supported");
    CHECK_CONDITION(faces <= 1 && depth <= 1 &&
                    "3D or cube KTX files not supported");
    CHECK_CONDITION(
        mipmaps != 0 &&
        "KTX files without explicitly specified mipmap count not supported");
    ...
    CHECK_CONDITION(format_ != TextureFormat::UNKNOWN &&
                    "Unsupported texture format in KTX file");
    idx += keyValueBytes;
    ...
    for (unsigned i = 0; i < mipmaps; ++i) {
        ...
        CHECK_CONDITION(levelSize + dataOffset <= imgDataSize_ &&
                        "KTX mipmap level data size exceeds file size");
...
void Image::ReadPVR() {
    ...
    CHECK_CONDITION(faces <= 1 && depth <= 1 &&
                    "3D or cube PVR files not supported");
    CHECK_CONDITION(
        mipmaps != 0 &&
        "PVR files without explicitly specified mipmap count not supported");
    format_ = TextureFormat::UNKNOWN;
...
Image::CompressedLevel Image::GetCompressedLevel(const unsigned char* data,
                                                 unsigned dataSize,
    ...
    if (format_ >= TextureFormat::DXT1 && format_ <= TextureFormat::ETC1) {
        level.blockSize_ =
            format_ == TextureFormat::DXT1 || format_ == TextureFormat::ETC1
                ? 8
                : 16;
        unsigned i = 0;
        unsigned offset = 0;
        ...
        for (;;) {
            ...
            int dataWidth =
                std::max(level.width_, level.blockSize_ == 2 ? 16 : 8);
            int dataHeight = std::max(level.height_, 8);
            level.data_ = data + offset;
            level.dataSize_ =
                (dataWidth * dataHeight * level.blockSize_ + 7) >> 3;
...
void Image::FlipBlockVertical(unsigned char* dest, const unsigned char* src,
                              TextureFormat format) {
    CHECK_ASSERT(format == TextureFormat::DXT1 ||
    ...
    case TextureFormat::DXT5:
        ...
        {
            unsigned a1 =
                src[2] | ((unsigned)src[3] << 8) | ((unsigned)src[4] << 16);
            unsigned a2 =
                src[5] | ((unsigned)src[6] << 8) | ((unsigned)src[7] << 16);
...
bool Image::FlipVertical() {
    if (compressed_) {
        if (format_ == TextureFormat::DXT1 || format_ == TextureFormat::DXT3 ||
            format_ == TextureFormat::DXT5) {
            unsigned dataOffset = 0;
            ...
            for (unsigned i = 0; i < numCompressedLevels_; ++i) {
                CompressedLevel level =
                    GetCompressedLevel(imgData_, imgDataSize_, i);
                ...
                for (unsigned y = 0; y < level.rows_; ++y) {
                    ...
                    unsigned char* dest =
                        inverted + dataOffset +
                        (level.rows_ - y - 1) * level.rowSize_;
                    for (unsigned x = 0; x < level.rowSize_;
                         x += level.blockSize_)
        ...
        unsigned char* inverted =
            (unsigned char*)malloc(channels_ * width_ * height_);
        if (!inverted)
            return false;
...
void Image::Resize2PowerOf2(const unsigned char*& imgData, int& width,
    ...
    up_scale_image(imgData, width, height, channels, resampled, new_width,
                   new_height);
...
void Image::Reduce(int size) {
    CHECK_CONDITION(!compressed_ &&
                    "Reduce not supported for compressed images!!!");
    ...
    if (reduceBlockX > 1 || reduceBlockY > 1) {
        ...
        unsigned char* newImgData =
            (unsigned char*)malloc(channels_ * newWidth * newHeight);
        mipmap_image(imgData_, width_, height_, channels_, newImgData,
                     reduceBlockX, reduceBlockY);
        free((void*)imgData_);
        width_ = newWidth;
        height_ = newHeight;
        imgData_ = newImgData;
...
bool Image::SaveAsPNG(const Path& outputDir) {
    CHECK_CONDITION_ARGS(imgData_ != nullptr &&
                             "Resource must be ready at this point!!!",
                         resource_->GetName());
    CHECK_ASSERT(!compressed_ &&
                 "Save for compressed image is not supported!!!");
    Path oPath;
    oPath.SetPath(outputDir.GetPath());
    oPath.SetName(Path(resource_->GetName()).GetName());
    oPath.SetExtension("png");
    int result = stbi_write_png(oPath.GetFullAbsoluteFilePath().c_str(), width_,
                                height_, channels_, imgData_, 0);
    if (!result) {
        LOGE("Error %s writting file %s", std::strerror(errno),
             oPath.GetFilePath().c_str());
        return false;
    }
    return true;
}
