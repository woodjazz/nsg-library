...
#include <cstring>
namespace NSG {
template <typename T> const char* ToString(T obj, const char* mapping[]) {
    auto idx = (int)obj;
    CHECK_ASSERT(idx >= 0 && idx < (int)T::MAX_INDEX);
    return mapping[idx];
}

template <typename T> T ToType(const char* value, const char* mapping[]) {
...
std::string ToString(const BoundingBox& obj) {
    ...
    const int MaxBuffer = 100;
    char buffer[MaxBuffer];
    snprintf(buffer, MaxBuffer, "[%g,%g,%g][%g,%g,%g]", obj.min_.x, obj.min_.y,
             obj.min_.z, obj.max_.x, obj.max_.y, obj.max_.z);
    return buffer;
}
...
BoundingBox ToBoundigBox(const std::string& buffer) {
    Vector3 min;
    Vector3 max;
    sscanf(buffer.c_str(), "[%g,%g,%g][%g,%g,%g]", &min.x, &min.y, &min.z,
           &max.x, &max.y, &max.z);
...
std::string ToString(const Matrix4& m) {
    ...
    char buffer[MaxBuffer];
    snprintf(buffer, MaxBuffer, "[ %s %s %s %s ]",
             ToString(m.Column(0)).c_str(), ToString(m.Column(1)).c_str(),
             ToString(m.Column(2)).c_str(), ToString(m.Column(3)).c_str());
    return buffer;
}
...
Matrix4 ToMatrix4(const std::string& buffer) {
    ...
    sscanf(buffer.c_str(), "[ %s %s %s %s ]", buffer0, buffer1, buffer2,
           buffer3);
    return Matrix4(ToVertex4(buffer0), ToVertex4(buffer1), ToVertex4(buffer2),
                   ToVertex4(buffer3));
}
...
int ToInt(const std::string& value) { return ToInt(value.c_str()); }
...
static const char* RenderPassStr[] = {"VERTEXCOLOR", "UNLIT",        "LIT",
                                      "TEXT",        "BLEND",        "BLUR",
                                      "WAVE",        "SHOW_TEXTURE0"};
...
static const char* TextureBlendStr[] = {
    "NONE",    "MIX",       "MUL",       "ADD",       "SUB",
    "DIV",     "DARK",      "DIFF",      "LIGHT",     "SCREEN",
    "OVERLAY", "BLEND_HUE", "BLEND_SAT", "BLEND_VAL", "BLEND_COLOR"};
...
static const char* TextureTypeStr[] = {
    "UNKNOWN", "COL",  "NORM",     "COLSPEC", "COLMIR", "VARS",
    "REF",     "SPEC", "EMIT",     "ALPHA",   "HAR",    "RAYMIRR",
    "TRANSLU", "AMB",  "DISPLACE", "WARP",    "LAYER"};
...
static const char* CullFaceModeStr[] = {"BACK", "FRONT", "FRONT_AND_BACK",
                                        "DISABLED"};
...
static const char* BLEND_MODEStr[] = {"NONE", "ALPHA", "MULTIPLICATIVE",
                                      "ADDITIVE"};
...
static const char* FillModeStr[] = {"SOLID", "WIREFRAME", "POINT"};
...
static const char* LightTypeStr[] = {"POINT", "DIRECTIONAL", "SPOT"};
...
static const char* PhysicsShapeStr[] = {
    "SH_UNKNOWN",        "SH_BOX",        "SH_CONE_Z",     "SH_CONE_Y",
    "SH_CONE_X",         "SH_CYLINDER_Z", "SH_CYLINDER_Y", "SH_CYLINDER_X",
    "SH_CAPSULE_Z",      "SH_CAPSULE_Y",  "SH_CAPSULE_X",  "SH_SPHERE",
    "SH_CONVEX_TRIMESH", "SH_TRIMESH"};
...
static const char* TextureWrapModeStr[] = {"CLAMP_TO_EDGE", "MIRRORED_REPEAT",
                                           "REPEAT"};
...
static const char* TextureFilterModeStr[] = {"NEAREST", "BILINEAR", "REPEAT"};
