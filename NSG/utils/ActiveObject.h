...
namespace utils
{
    ...
    class ActiveObject
    {
        ...
        struct Task : public BasicTask
        {
            ...
            Task(std::function<Res(Params... params)> func)
                : task_(func) {}
        ...
        struct PriorityTask
        {
            ...
            PriorityTask(const TimePoint& timePoint, Milliseconds duration, const std::shared_ptr<BasicTask>& task)
                : timePoint_(timePoint), duration_(duration), task_(task) {}
            bool operator > (const PriorityTask& obj) const
            {
                Milliseconds diff = std::chrono::duration_cast<Milliseconds>(timePoint_ - obj.timePoint_);
                return diff.count() > 0;
            }
        };
    public:
        ...
        std::future<Res> Enqueue(Fun func, Args... args)
        {
            ...
            auto task = std::unique_ptr<Task<Res>>(new Task<Res>(std::bind(func, args...)));
        ...
        template <typename Res, typename ... Args, typename Fun>
        std::future<Res> Schedule(const TimePoint& timePoint, Milliseconds duration, Fun func, Args... args)
        {
            std::lock_guard<std::mutex> guard(mtx_);
    ...
    private:
        ...
        std::priority_queue<PriorityTask, std::vector<PriorityTask>, std::greater<PriorityTask>> priorityQueue_;
    };
}
