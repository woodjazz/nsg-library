...
CameraControl::CameraControl(PCamera camera, PScene scene)
    : camera_(camera), scene_(scene),
      ...
      enableDebugPhysics_(false), enableColorSplits_(false), enabled_(true),
      selection_(false), selectionContext_(RendererContext::DEFAULT) {
    CHECK_ASSERT(camera_);
    CHECK_ASSERT(scene_);
    lastX_ = lastY_ = 0;
    pointOnSphere_ = PPointOnSphere(
        new PointOnSphere(Vertex3(0), camera_->GetGlobalPosition()));
    ...
    slotWindow_ = camera->SigWindow()->Connect(
        [this](PWindow window) { SetWindow(window); });
...
void CameraControl::SetWindow(PWindow window) {
    if (window_.lock() != window) {
        ...
        if (window) {
            slotMouseMoved_ =
                window->SigFloatFloat()->Connect([&](float x, float y) {
            ...
            slotMouseWheel_ = window->SigMouseWheel()->Connect(
                [&](float x, float y) { OnMousewheel(x, y); });

            slotMultiGesture_ = window->SigMultiGesture()->Connect(
                [&](int timestamp, float x, float y, float dTheta, float dDist,
                    int numFingers) {
...
void CameraControl::OnUpdate(float deltaTime) {
    ...
    if (trackNode_ &&
        pointOnSphere_->SetCenter(trackNode_->GetGlobalPosition()))
        updateOrientation_ = true;

    if (updateOrientation_) {
        Quaternion targetOrientation = camera_->GetLookAtOrientation(
            pointOnSphere_->GetCenter(), pointOnSphere_->GetUp());
        ...
        if (!close) {
            ...
            camera_->SetOrientation(
                currentOrientation.Slerp(targetOrientation, factor));
        } else {
...
void CameraControl::OnMultiGesture(int timestamp, float x, float y,
...
void CameraControl::Move(float x, float y) {
    ...
    if (altKeyDown_ && shiftKeyDown_) {
        ...
        camera_->SetGlobalLookAtPosition(pointOnSphere_->GetCenter(),
                                         pointOnSphere_->GetUp());
...
void CameraControl::Track(PNode node) { trackNode_ = node; }
...
PSceneNode CameraControl::SelectObject(float x, float y) {
    ...
    if (selection) {
        ...
        return std::dynamic_pointer_cast<SceneNode>(
            SharedFromPointerNode(closest.node_));
    }
    return nullptr;
}
...
void CameraControl::Enable(bool enable) { enabled_ = enable; }
