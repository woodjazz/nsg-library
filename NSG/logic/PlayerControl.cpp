...
PlayerControl::PlayerControl(PWindow window)
    : signalMoved_(new SignalFloatFloat),
      ...
      signalButtonA_(new SignalBool), leftHorizontalAxis_(0),
      leftVerticalAxis_(0), rightHorizontalAxis_(0), rightVerticalAxis_(0),
      left_(false), right_(false), forward_(false), backward_(false),
      leftFingerId_{false, 0}, rightFingerId_{false, 0} {
    SetWindow(window);

    slotUpdate_ = Engine::SigUpdate()->Connect(
...
void PlayerControl::SetWindow(PWindow window) {
    if (window_.lock() != window) {
        ...
        if (window) {
            slotMouseMoved_ = window->SigFloatFloat()->Connect(
                [this](float x, float y) { OnMousemoved(x, y); });

            slotMouseDown_ = window->SigMouseDown()->Connect([this](
                int button, float x, float y) { OnMouseDown(button, x, y); });

            slotMouseUp_ = window->SigMouseUp()->Connect([this](
                int button, float x, float y) { OnMouseUp(button, x, y); });

            slotMouseWheel_ = window->SigMouseWheel()->Connect(
                [this](float x, float y) { OnMousewheel(x, y); });

            slotMultiGesture_ = window->SigMultiGesture()->Connect(
                [this](int timestamp, float x, float y, float dTheta,
            ...
            slotJoystickDown_ = window->SigJoystickDown()->Connect(
                [this](int joystickID, JoystickButton button) {
                    if (JoystickButton::BUTTON_A == button)
                        signalButtonA_->Run(true);

                });

            slotJoystickUp_ = window->SigJoystickUp()->Connect(
                [this](int joystickID, JoystickButton button) {
                    if (JoystickButton::BUTTON_A == button)
                        signalButtonA_->Run(false);
                });

            slotJoystickAxisMotion_ = window->SigJoystickAxisMotion()->Connect(
                [this](int joystickID, JoystickAxis axis, float position) {
                    switch (axis) {
            ...
            slotTouchFinger_ = window->SigTouchFinger()->Connect(
                [this](const TouchFingerEvent& event) {
                    // LOGW("x=%f y=%f touchId=%lld fingerId=%lld", event.x,
                    // event.y, event.touchId, event.fingerId);
                    ...
                    if (event.type == TouchFingerEvent::Type::DOWN) {
                        ...
                        if (leftFingerId_.first &&
                            leftFingerId_.second == event.fingerId) {
                            leftHorizontalAxis_ += event.dx;
                            leftVerticalAxis_ -= event.dy;
                        } else if (rightFingerId_.first &&
                                   rightFingerId_.second == event.fingerId) {
...
void PlayerControl::OnMultiGesture(int timestamp, float x, float y,
