...
- (void)layoutSubviews {
    uint32_t frameW =
        (uint32_t)(self.contentScaleFactor * self.frame.size.width);
    uint32_t frameH =
        (uint32_t)(self.contentScaleFactor * self.frame.size.height);
...
- (void)start {
    if (nil == displayLink_) {
        displayLink_ =
            [self.window.screen displayLinkWithTarget:self
...
- (void)touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event {
    ...
    // s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x,
    // touchLocation.y, 0);
    // s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x,
    // touchLocation.y, 0, MouseButton::Left, true);
}
...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
    ...
    CGPoint touchLocation = [touch locationInView:self];
    // s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x,
    // touchLocation.y, 0, MouseButton::Left, false);
}
...
- (void)createContext {
    // float scaleFactor = [[UIScreen mainScreen] scale]; // should use this,
    // but ui is too small on ipad retina
    ...
    CHECK_ASSERT(nullptr != context_ &&
                 "Failed to create kEAGLRenderingAPIOpenGLES2 context.");
...
- (void)createFrameBuffers {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8_OES, width,
                          height); // from OES_packed_depth_stencil
...
- (void)applicationWillResignActive:(UIApplication*)application {
    // s_ctx->m_eventQueue.postSuspendEvent(s_defaultWindow,
    // Suspend::WillSuspend);
    [view_ stop];
}
...
IOSWindow::IOSWindow(const std::string& name, int x, int y, int width,
...
IOSWindow::~IOSWindow() { Close(); }

void IOSWindow::Initialize(int x, int y, int width, int height,
                           WindowFlags flags) {
    Window::SetMainWindow(this);
}

void IOSWindow::Close() { Window::Close(); }
...
void IOSWindow::SetupImgui() { Window::SetupImgui(); }
...
int IOSWindow::RunApplication() {
    ...
    int exitCode = UIApplicationMain(0, nullptr, @"UIApplication",
