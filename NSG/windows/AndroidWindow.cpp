...
AndroidWindow::AndroidWindow(const std::string& name, WindowFlags flags)
    : Window(name), flags_(0), sensorManager_(nullptr),
      accelerometerSensor_(nullptr), sensorEventQueue_(nullptr),
...
AndroidWindow::AndroidWindow(const std::string& name, int x, int y, int width,
...
AndroidWindow::~AndroidWindow() { Close(); }
...
static int GetUnicodeChar(ANativeActivity* pActivity, int eventType,
    ...
    if (metaState == 0) {
        jmethodID method_get_unicode_char =
            jniEnv->GetMethodID(class_key_event, "getUnicodeChar", "()I");
        ...
        unicodeKey =
            jniEnv->CallIntMethod(eventObj, method_get_unicode_char, metaState);
    }
...
int32_t AndroidWindow::HandleInput(struct android_app* app,
                                   AInputEvent* event) {
    ...
    case AINPUT_EVENT_TYPE_MOTION: {
        ...
        int32_t index =
            (actionBits & AMOTION_EVENT_ACTION_POINTER_INDEX_MASK) >>
            AMOTION_EVENT_ACTION_POINTER_INDEX_SHIFT;

        // Simulate left mouse click with 1st touch and right mouse click with
        // 2nd touch. ignore other touchs
        if (count < 2) {
            switch (action) {
            case AMOTION_EVENT_ACTION_DOWN:
            case AMOTION_EVENT_ACTION_POINTER_DOWN:
                window->OnMouseDown(action == AMOTION_EVENT_ACTION_DOWN
                                        ? NSG_BUTTON_LEFT
                                        : NSG_BUTTON_RIGHT,
            ...
            case AMOTION_EVENT_ACTION_POINTER_UP:
                window->OnMouseUp(action == AMOTION_EVENT_ACTION_UP
                                      ? NSG_BUTTON_LEFT
                                      : NSG_BUTTON_RIGHT,
                                  (int32_t)mx, (int32_t)my);
                break;
    ...
    case AINPUT_EVENT_TYPE_KEY: {
        ...
        window->OnKey(GetTranslatedKeyCode(keyCode), action,
                      GetTranslatedModifier(modifiers));
        ...
        if (NSG_KEY_PRESS == action) {
            if (keyCode == AKEYCODE_BACK) {
                ...
                window->Close();
                exit(0);
            } else if (keyCode != AKEYCODE_SHIFT_LEFT &&
                       keyCode != AKEYCODE_SHIFT_RIGHT) {
                window->OnChar(
                    GetUnicodeChar(app->activity, action, keyCode, modifiers));
            }
        }
        return 1;
    } break;
...
void AndroidWindow::HandleCmd(struct android_app* app, int32_t cmd) {
    ...
    case APP_CMD_GAINED_FOCUS:
        ...
        if (window->accelerometerSensor_) {
            ASensorEventQueue_enableSensor(window->sensorEventQueue_,
                                           window->accelerometerSensor_);
            // We'd like to get 60 events per second (in us).
            ASensorEventQueue_setEventRate(window->sensorEventQueue_,
                                           window->accelerometerSensor_,
                                           (1000L / 60) * 1000);
        }
        break;
    ...
    case APP_CMD_LOST_FOCUS:
        ...
        if (window->accelerometerSensor_)
            ASensorEventQueue_disableSensor(window->sensorEventQueue_,
                                            window->accelerometerSensor_);
        break;
...
bool AndroidWindow::IsValid() { return surface_ != EGL_NO_SURFACE; }

void AndroidWindow::Initialize(int x, int y, int width, int height,
                               WindowFlags flags) {
    ...
    sensorEventQueue_ = ASensorManager_createEventQueue(
        sensorManager_, androidApp->looper, LOOPER_ID_USER, nullptr, nullptr);
    Window::SetMainWindow(this);
}

void AndroidWindow::Close() { Window::Close(); }

void AndroidWindow::RestoreContext() {
    if (eglGetCurrentContext() == EGL_NO_CONTEXT) {
        // On Android the context may be lost behind the scenes as the
...
void AndroidWindow::SetContext() {
    if (display_ != EGL_NO_DISPLAY &&
        eglMakeCurrent(display_, surface_, surface_, eglContext_) ==
            EGL_FALSE) {
...
void AndroidWindow::InitDisplay() {
    if (androidApp->window && isMainWindow_ && surface_ == EGL_NO_SURFACE) {
        ...
        surface = eglCreateWindowSurface(display, config, androidApp->window,
                                         nullptr);
...
void AndroidWindow::SwapWindowBuffers() { eglSwapBuffers(display_, surface_); }
...
void AndroidWindow::HandleEvents() {
    ...
    if (ident >= 0) {
        ...
        if (ident == LOOPER_ID_USER) {
            if (window->accelerometerSensor_) {
                ASensorEvent event;
                while (ASensorEventQueue_getEvents(window->sensorEventQueue_,
                                                   &event, 1) > 0) {
...
void AndroidWindow::SetupImgui() { Window::SetupImgui(); }
